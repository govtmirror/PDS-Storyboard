 ...
 * - added logic for updating the creator of the initial categories
 *
 * @package cubetboard
 * @version 2.1
...
var populateDbData = require('../../data_generator/populate-db-data');
...
process.on('uncaughtException', function (exception) {
  ...
  fs.remove(path.join(installDir, '_install'), function (err) {
    if (err) {
      console.log(err);
    }
...
module.exports = function (app, sFolderPath, directory) {
  ...
  function insertdta (db_sample, db_name, user_data, res, db, files, mongo, callback) {
    ...
    async.waterfall([
      function (cb) {   // drop adminuser and site_settings
        async.each(['adminuser', 'site_settings'], function (collection, cb2) {
          var model = db.collection(collection);
          model.drop(cb2);
        }, function () {
          logProgress('Dropped adminuser and site_settings.');
          cb();
        });
      },
      function (cb) {   // drop all collection
        populateDbData.dropAllCollections(db, cb);
      },
      function (cb) { // copy site_logo
      ...
      function (cb) {   // create loginpages
        populateDbData.createLoginPages(db, cb);
      },
      function (cb) {   // create roles
        populateDbData.createRoles(db, cb);
      },
      function (cb) {   // create test data
        if (db_sample) {
          populateDbData.createTestData(db, cb);
        } else {
          cb();
        }
      },
      function (cb) {   // create adminuser
        var collection = db.collection('adminuser');
        collection.insert(user_data.user_data, function (err, inserted_data) {
          if (!err) {
            logProgress('adminuser inserted !');
          }
          cb(err);
        });
      },
      function (cb) {   // create site_settings
        var collection = db.collection('site_settings');
        collection.insert(user_data.settings, function (err, inserted_data) {
          if (!err) {
            logProgress('settings inserted !');
          }
          cb(err);
        });
      }
      ...
      if (err) {
        console.log('Error on install:', err);
      } else {
        var err = fs.copySync(sFolderPath, path.join(directory, '/_install'));
        if (err) {
          throw err;
          sio.sockets.emit('copy_err');
          console.log('Please copy your "install" directory to "_install" directory');
        } else {
          console.log('install directory copy to _install');
        }
        var send_data = {
          "res": 1
        };
